...
function ClientExpenseView({isInvoice}) {
  ...
  return (
    <PageWrapper title= {isInvoice?"Client Expense Invoices":"Client Expense Receipts"}>
        <Grid>
          <Grid.Col span={12}>
            {isInvoice?
            <Flex justify='end'>
                <Button leftIcon={<CirclePlus/>} variant="filled">Add Expense</Button>
            </Flex>:"Filter for reciepts"
            }
          </Grid.Col>
          <Grid.Col span={12}>
            {loading ? <Loader/> :
              clientExpenseData.length > 0 ?
                <DataGrid columns={ClientExpenseViewHeader(true)} data={clientExpenseData} pagination={true} />
                : <Text align='center'>
                    {isInvoice?
